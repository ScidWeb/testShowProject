  ...
  (Sifter.prototype.tokenize = function (query) {
    ...
    (Sifter.prototype.getScoreFunction = function (search, options) {
      function scoreValue(score, token) {
        var pos;
        return !score || -1 === (pos = (score = String(score || '')).search(token.regex))
          ? 0
          : ((score = token.string.length / score.length), 0 === pos && (score += 0.5), score);
      }
      var field_count,
        ...
        ? 1 === fields_count
          ...
          : function (a, b) {
              for (var result, i = 0; i < fields_count; i++)
                if (((result = fields[i].field), (result = multipliers[i] * cmp(get_field(result, a), get_field(result, b)))))
                  return result;
              return 0;
            }
        : null;
    ...
    asciifold = (function () {
      ...
      for (k in DIACRITICS)
        if (DIACRITICS.hasOwnProperty(k))
          for (foreignletters += chunk = DIACRITICS[k].substring(2, DIACRITICS[k].length - 1), i = 0, n = chunk.length; i < n; i++)
  ...
  (function (root, factory) {
    ...
    function measureString(str, $parent) {
      return str
        ? (Selectize.$testInput ||
            ...
            $('<div />')
              .css({ position: 'absolute', width: 0, height: 0, overflow: 'hidden' })
            ...
            e.type &&
              ...
              46 === keyCode || 8 === keyCode
                ? (selection = getSelection($input[0])).length
                  ? (width = width.substring(0, selection.start) + width.substring(selection.start + selection.length))
                  : 8 === keyCode && selection.start
                  ? (width = width.substring(0, selection.start - 1) + width.substring(selection.start + 1))
                  : 46 === keyCode &&
                    void 0 !== selection.start &&
                    (width = width.substring(0, selection.start) + width.substring(selection.start + 1))
                : shift &&
                  ((shift = e.shiftKey),
                  (placeholder = String.fromCharCode(e.keyCode)),
                  (width += placeholder = shift ? placeholder.toUpperCase() : placeholder.toLowerCase()))),
            (placeholder = $input.attr('placeholder')),
            (width = measureString((width = !width && placeholder ? placeholder : width), $input) + 4) !== currentWidth &&
    ...
    var IS_MAC = /Mac/.test(navigator.userAgent),
      ...
      Selectize = function ($input, settings) {
        ...
        if (
          ((dir =
            (dir = dir ? dir.getPropertyValue('direction') : input.currentStyle && input.currentStyle.direction) ||
    ...
    return (
      ...
      void 0 !== MicroPlugin
        ...
        : (function (message, options) {
            ...
            console.error('Selectize: ' + message),
              options.explanation &&
                (console.group && console.group(), console.error(options.explanation), console.group && console.groupEnd());
      ...
      $.extend(Selectize.prototype, {
        setup: function () {
          ...
          (inputId = $input.attr('id')) &&
            ($control_input.attr('id', inputId + '-selectized'), $("label[for='" + inputId + "']").attr('for', inputId + '-selectized')),
            ...
            self.plugins.names.length &&
              ((delimiterEscaped = 'plugin-' + self.plugins.names.join(' plugin-')),
          ...
          this.settings.render = $.extend(
            {},
            {
              ...
              option: function (data, escape) {
                return (
                  '<div class="option ' +
                  ('' === data[field_value] ? 'selectize-dropdown-emptyoptionlabel' : '') +
                  '">' +
        ...
        onMouseDown: function (e) {
          ...
          if (self.isFocused) {
            if (e.target !== self.$control_input[0])
              return (
                'single' === self.settings.mode ? (self.isOpen ? self.close() : self.open()) : defaultPrevented || self.setActiveItem(null),
                !1
              );
          } else
            defaultPrevented ||
        ...
        onKeyPress: function (e) {
          ...
          return this.settings.create && 'multi' === this.settings.mode && character === this.settings.delimiter
        ...
        onKeyUp: function (value) {
          ...
          this.lastValue !== value &&
            ((this.lastValue = value), this.onSearchChange(value), this.refreshOptions(), this.trigger('type', value));
        },
            ...
            (value = $(e.currentTarget)).hasClass('create')
              ...
              : void 0 !== (value = value.attr('data-value')) &&
                ...
                self.settings.closeAfterSelect
                  ? self.close()
                  : !self.settings.hideSelected && e.type && /mouse/.test(e.type) && self.setActiveOption(self.getOption(value)));
        },
        ...
        setActiveItem: function ($item, e) {
          ...
          if ('single' !== this.settings.mode) {
            if (!($item = $($item)).length)
              return $(this.$activeItems).removeClass('active'), (this.$activeItems = []), void (this.isFocused && this.showInput());
            if ('mousedown' === (idx = e && e.type.toLowerCase()) && this.isShiftDown && this.$activeItems.length) {
              for (
                swap = this.$control.children('.active:last'),
                  ...
                  (end = Array.prototype.indexOf.apply(this.$control[0].childNodes, [$item[0]])) < begin &&
                ...
                (item = this.$control[0].childNodes[i]),
                  -1 === this.$activeItems.indexOf(item) && ($(item).addClass('active'), this.$activeItems.push(item));
              e.preventDefault();
            } else
              ('mousedown' === idx && this.isCtrlDown) || ('keydown' === idx && this.isShiftDown)
                ? $item.hasClass('active')
                  ? ((idx = this.$activeItems.indexOf($item[0])), this.$activeItems.splice(idx, 1), $item.removeClass('active'))
        ...
        setActiveOption: function (scroll_bottom, scroll, animate) {
          ...
          this.$activeOption &&
            (this.$activeOption.removeClass('active'), this.trigger('dropdown_item_deactivate', this.$activeOption.attr('data-value'))),
            ...
            (scroll_bottom = $(scroll_bottom)).length &&
              ...
              (!scroll && isset(scroll)) ||
                ...
                (scroll_bottom =
                  (scroll_top = y = this.$activeOption.offset().top - this.$dropdown_content.offset().top + scroll) -
                  height_menu +
                  height_item),
                height_menu + scroll < y + height_item
                  ...
                  : y < scroll &&
                    this.$dropdown_content.stop().animate({ scrollTop: scroll_top }, animate ? this.settings.scrollDuration : 0)));
        },
        selectAll: function () {
        ...
        refreshOptions: function (triggerDropdown) {
          ...
          if (
            ...
            self.settings.highlight && ($dropdown_content.removeHighlight(), results.query.length && results.tokens.length))
          )
            ...
            self.hasOptions
              ? (0 < results.items.length
                  ? (($active_before = $active_before && self.getOption($active_before)),
                    '' !== results.query && $active_before && $active_before.length
        ...
        addOption: function (data) {
          ...
          if (Array.isArray(data)) for (i = 0, n = data.length; i < n; i++) this.addOption(data[i]);
            ...
            (value = this.registerOption(data)) &&
              ((this.userOptions[value] = !0), (this.lastQuery = null), this.trigger('option_add', value, data));
        },
        ...
        updateOption: function ($item, $item_new) {
          ...
          if (
            ...
            (cache_options = this.options[$item].$order),
              ...
              cache_items && (delete cache_items[$item], delete cache_items[value_new]),
              cache_options && (delete cache_options[$item], delete cache_options[value_new]),
        ...
        getElementWithValue: function (value, $els) {
          if (null != (value = hash_key(value)))
            for (var i = 0, n = $els.length; i < n; i++) if ($els[i].getAttribute('data-value') === value) return $($els[i]);
          return $();
        },
        getElementWithTextContent: function (textContent, ignoreCase, $els) {
          if (null != (textContent = hash_key(textContent)))
            for (var i = 0, n = $els.length; i < n; i++) {
              ...
              if (
                (1 == ignoreCase &&
                  ((eleTextContent = null !== eleTextContent ? eleTextContent.toLowerCase() : null),
              ...
              'function' == typeof self.settings.create
                ...
                : function (key) {
                    ...
                    if (
                      ...
                      (null == (key = self.settings.formatValueToKey.apply(this, [key])) ||
                        'object' == typeof key ||
                        'function' == typeof key)
                    )
                      throw new Error(
        ...
        positionDropdown: function () {
          ...
          (offset.top += $control.outerHeight(!0)),
            this.$dropdown.css({ width: $control[0].getBoundingClientRect().width, top: offset.top, left: offset.left });
        },
        ...
        deleteSelection: function (e) {
          ...
          if (
            (this.$activeOption &&
              ...
              (option_select = ('string' == typeof this.settings.deselectBehavior && 'top' === this.settings.deselectBehavior
                ? this.getFirstOption()
            ...
            for (
              caret = this.$control.children('.active:' + (0 < direction ? 'last' : 'first')),
          ...
          0 !== direction &&
            ...
            this.isFocused && !this.isInputHidden
              ? ((valueLength = this.$control_input.val().length),
                (direction < 0 ? 0 !== selection.start || 0 !== selection.length : selection.start !== valueLength) ||
                  valueLength ||
        ...
        advanceCaret: function (direction, e) {
          ...
          0 !== direction &&
            (this.isShiftDown
              ? ($adj = this.$control_input[0 < direction ? 'next' : 'prev']()).length &&
        ...
        render: function (templateName, data) {
          var value,
            ...
            cache = !1;
          return (cache =
            'option' === templateName || 'item' === templateName ? !!(value = hash_key(data[this.settings.valueField])) : cache) &&
      ...
      ($.fn.selectize = function (settings_user) {
        function init_select($input, settings_element) {
          ...
          function addOption($option, group) {
            ...
            (value || settings.allowEmptyOption) &&
              (optionsMap.hasOwnProperty(value)
                ...
                : (((option = readData($option) || {})[field_label] = option[field_label] || $option.text()),
          ...
          var i,
            ...
            optionsMap = {};
          for (
            settings_element.maxItems = $input.attr('multiple') ? null : 1, i = 0, n = ($children = $input.children()).length;
            i < n;
            i++
          )
        ...
        return this.each(function () {
          ...
          this.selectize ||
            ...
            (settings_element = $input.attr('placeholder') || $input.attr('data-placeholder')) ||
      ...
      Selectize.define('optgroup_columns', function (options) {
                ...
                : ((self.ignoreHover = !0),
                  ...
                  void (
                    ($option = ($options = ($options =
                      37 === e.keyCode ? $options.prev('[data-group]') : $options.next('[data-group]')).find('[data-selectable]')).eq(
                      Math.min($options.length - 1, $option)
                    )).length && this.setActiveOption($option)
                  ));
            }));
        function equalizeSizes() {
          ...
          if (n && self.$dropdown_content.width()) {
            ...
            options.equalizeWidth &&
              ...
              1 < n && ((width_last = width_parent - width_last * (n - 1)), $optgroups.eq(n - 1).css({ width: width_last })));
          }
        }
        ...
        (options.equalizeHeight || options.equalizeWidth) &&
          (hook.after(this, 'positionDropdown', equalizeSizes), hook.after(this, 'refreshOptions', equalizeSizes));
      }),
            ...
            function (e) {
              ...
              return 8 === e.keyCode &&
                ...
                ? ((option = this.options[this.items[option]]),
                  this.deleteSelection(e) && (this.setTextboxValue(options.text.apply(this, [option])), this.refreshOptions(!0)),
                  void e.preventDefault())
                : original.apply(this, arguments);
            }));
      }),
